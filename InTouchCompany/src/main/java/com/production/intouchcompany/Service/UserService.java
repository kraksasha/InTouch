...
import com.production.intouchcompany.Entity.*;
import com.production.intouchcompany.Repository.FriendRepository;
import com.production.intouchcompany.Repository.PhotoRepository;
import com.production.intouchcompany.Repository.UserRepository;
...
public class UserService implements UserDetailsService {
    ...
    private List<User> getSearchByTown(List<User> list, String town){
        List<User> resultList = new ArrayList<>();
        for (int i = 0; i < list.size(); i++){
            if (list.get(i).getTown().equalsIgnoreCase(town)){
                resultList.add(list.get(i));
            }
        }
        return resultList;
